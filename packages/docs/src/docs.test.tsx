...
describe('Docs Application Shell', () => {
  ...
  it('renders the header title and navigation links', () => {
    ...
    expect(screen.getByRole('link', { name: 'Styling' })).toBeDefined();
    expect(screen.getByRole('link', { name: 'Localization' })).toBeDefined();
    expect(screen.getByRole('link', { name: 'Icons' })).toBeDefined();
  });
