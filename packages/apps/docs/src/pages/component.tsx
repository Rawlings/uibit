...
import type { DualCode } from '../types/docs';
...
function ExampleBlock({ example }: { example: any }) {
  ...
  // biome-ignore lint/correctness/useExhaustiveDependencies: example.Demo is intentionally unused in the body — it re-triggers this effect when the rendered demo changes
  useEffect(() => {
    if (ref.current) {
...
export default function ComponentDocs() {
  ...
  const tocItems = useMemo(
    () =>
      comp
        ? [
            { id: 'demo', label: 'Demo', show: !!comp.Demo },
            {
              id: 'readme',
              label: 'Readme',
              show: !comp.Demo && !!comp.readme,
            },
            {
              id: 'api-reference',
              label: 'API Reference',
            ...
            },
            {
              id: 'examples',
              label: 'Examples',
              show: !!(comp.examples && comp.examples.length > 0),
            },
            {
              id: 'usage',
              label: 'Usage',
              show: !comp.examples && !!(comp.usages && comp.usages.length > 0),
            },
            { id: 'accessibility', label: 'Accessibility', show: !!comp.a11y },
            {
              id: 'features',
              label: 'Features',
              show: !!(comp.features && comp.features.length > 0),
            },
    ...
    return (
      <div className="max-w-7xl mx-auto px-4 py-16 text-center bg-white dark:bg-gray-950 text-gray-900 dark:text-gray-100 transition-colors duration-200">
        ...
        <p className="text-gray-600 dark:text-gray-400 mb-6">
          The requested component documentation does not exist.
        </p>
        <Link
          to="/"
      ...
      <div className="flex flex-col md:flex-row gap-8">
        ...
        <div className="flex-1 min-w-0">
          ...
          {!Demo && comp.readme && (
            <section
              id="readme"
              className="py-10 scroll-mt-20 prose dark:prose-invert max-w-none"
            >
              ...
              <div className="space-y-8">
                {usages.map((usage, index) => (
                  <div key={index}>
                    ...
                    {usage.description && (
                      <p className="text-sm text-gray-600 dark:text-gray-450 mb-3">
                        {usage.description}
                      </p>
                    )}
                      ...
                      <table className="w-full text-sm">
                        <thead>
                          <tr className="border-b border-gray-200 dark:border-gray-800">
                            <th className="py-2.5 text-left font-semibold text-gray-400 dark:text-gray-500 text-xs pr-4 last:pr-0">
                              Key
                            </th>
                            <th className="py-2.5 text-left font-semibold text-gray-400 dark:text-gray-500 text-xs pr-4 last:pr-0">
                              Function
                        ...
                        <tbody className="divide-y divide-gray-100 dark:divide-gray-900">
                          {comp.a11y.keyboardNav.map((kb, index) => (
                            <tr key={index} className="bg-transparent">
                              ...
                              <td className="py-3 text-left align-top text-gray-600 dark:text-gray-350 text-sm pr-4 last:pr-0">
                                {kb.description}
                              </td>
                            </tr>
                          ))}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {features.map((feature, i) => (
                  <div key={i} className="flex items-start gap-3">
                    ...
                    <span className="text-gray-750 dark:text-gray-350 text-sm">
                      {feature}
                    </span>
                  </div>
                ))}
          ...
          {comp.changelog && (
            <section
              id="changelog"
              className="py-10 scroll-mt-20 border-t border-gray-100 dark:border-gray-900"
            >
              <h2 className="text-xl font-semibold text-gray-900 dark:text-white mb-6">
                Changelog
              </h2>
              <div className="relative pl-6 border-l border-gray-200 dark:border-gray-800 space-y-8 ml-4 font-sans">
                {parseChangelog(comp.changelog).map((release, rIdx) => {
                  const categoryBadgeStyles: Record<string, string> = {
                    ...
                    Changed:
                      'bg-gray-50 text-gray-700 border-gray-200/50 dark:bg-gray-900/30 dark:text-gray-305 dark:border-gray-805/50',
                    Deprecated:
                      'bg-amber-50 text-amber-700 border-amber-200/50 dark:bg-amber-950/20 dark:text-amber-400 dark:border-amber-900/30',
                    Other:
                      'bg-gray-50 text-gray-600 border-gray-150 dark:bg-gray-900/20 dark:text-gray-400 dark:border-gray-800',
                  };
                  ...
                  return (
                    <div key={rIdx} className="relative">
                      ...
                      <div className="flex flex-wrap items-baseline gap-3 mb-3">
                        ...
                        <span className="text-xs text-gray-400 dark:text-gray-550 font-mono">
                          {release.date}
                        </span>
                      </div>
                      ...
                      <div className="space-y-5">
                        {release.sections.map((section, sIdx) => {
                          const badgeStyle =
                            categoryBadgeStyles[section.category] ||
                            categoryBadgeStyles.Other;
                          return (
