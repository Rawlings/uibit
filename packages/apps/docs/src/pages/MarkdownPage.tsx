      ...
      <table className="w-full text-sm text-left">
        ...
        <tbody className="divide-y divide-gray-200 dark:divide-gray-800">
          {localizedStrings.map((row, i) => (
            <tr key={i} className="bg-transparent hover:bg-gray-50/50 dark:hover:bg-gray-900/30 transition-colors">
              <td className="px-6 py-4 text-gray-900 dark:text-white font-medium">{row.component}</td>
              <td className="px-6 py-4 text-gray-600 dark:text-gray-350 font-mono text-xs">{row.string}</td>
              <td className="px-6 py-4 text-gray-500 dark:text-gray-400">{row.context}</td>
            </tr>
          ))}
        </tbody>
      </table>
