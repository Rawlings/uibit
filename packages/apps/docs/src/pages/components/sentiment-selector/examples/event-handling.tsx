  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <pre className="bg-gray-50 rounded p-4 text-sm text-gray-700 min-h-16">
        {lastEvent
          ? JSON.stringify(
              { event: lastEvent.type, detail: lastEvent.detail },
              null,
              2,
            )
          : 'No event fired yet.'}
...
const example: UsageExample = {
  ...
  description:
    'sentiment-change fires on every selection; sentiment-submit fires on re-tap for explicit confirmation. Both carry { value, label } in event.detail.',
