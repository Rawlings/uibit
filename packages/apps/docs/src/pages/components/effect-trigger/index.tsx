        ...
        for (let i = 0; i < 20; i++) {
          ...
          const anim = clone.animate(
            [
              {
                transform:
                  'translate(-50%, -50%) translate(0, 0) scale(1) rotate(0deg)',
                opacity: '1',
              },
              {
                transform: `translate(-50%, -50%) translate(${destX}px, ${destY}px) scale(0.3) rotate(${Math.random() * 360}deg)`,
                opacity: '0',
              },
            ],
            {
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="space-y-4">
            <div>
              ...
              >
                <option value="float-displace">
                  float-displace (Floating up)
                </option>
                <option value="fountain-burst">
                  fountain-burst (Physics fountain)
                </option>
                <option value="vortex-attractor">
                  vortex-attractor (Inward spiral)
                </option>
                <option value="ambient-drift">
                  ambient-drift (Rising bubbles)
                </option>
                <option value="focal-pop">focal-pop (Screen-center pop)</option>
                <option value="orbit-halo">orbit-halo (Circular ring)</option>
                <option value="traverse-x-right">
                  traverse-x-right (Left-to-right sweep)
                </option>
                <option value="traverse-x-left">
                  traverse-x-left (Right-to-left sweep)
                </option>
                <option value="traverse-y-up">
                  traverse-y-up (Bottom-to-top sweep)
                </option>
                <option value="traverse-y-down">
                  traverse-y-down (Top-to-bottom sweep)
                </option>
                <option value="custom-confetti">
            ...
            <div className="grid grid-cols-2 gap-4 items-center">
              <div>
                ...
                <input
                  ...
                  value={density}
                  onChange={(e) =>
                    setDensity(Math.max(1, parseInt(e.target.value, 10) || 1))
                  }
                  className="w-full text-sm bg-white border border-gray-200 rounded p-2 focus:outline-none focus:ring-2 focus:ring-black"
                />
              </div>
