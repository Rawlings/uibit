...
async function main() {
  ...
  for (const [tag, comp] of components.entries()) {
    ...
    const hasPrivateFields = comp.properties?.some(p => p.name.startsWith('_')) || false;
    const hasPrivateMethods = comp.methods?.some(m => m.name.startsWith('_')) || false;
    
    if (hasPrivateFields || hasPrivateMethods) {
      console.error(`❌ Error: Component <${tag}> contains private members starting with "_"!`);
      process.exit(1);
    }
  }

  console.log('✅ Success: ManifestScanner works and correctly filtered private members!');
